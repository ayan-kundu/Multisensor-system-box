...
void setup() {
  ...
  File myFile;
  myFile = SD.open("DT.txt", O_CREAT | O_APPEND | O_WRITE);
  delay(50);
  myFile.println();
...
void loop() {
  ...
  if (pushSwitchState == LOW) {
    ...
    outputstring=String("\tCo2:") + Co2_value + String(" CM:") + Moisture_Percent + String(" CT:") + temperature_Celsius + String(" AM:") + air_Moisture_Percent + String(" AT:") + air_temperature_Celsius;
    myFile.println(noww.timestamp(DateTime::TIMESTAMP_TIME)+outputstring);
    // Try avoid unnecessary variables and lines
    myFile.close();
    myFile.flush();
    //Serial.println("->");
    //Serial.println(String("\tCo2:") + Co2_value + String("ppm | Moisture: ") + Moisture_Percent + String("% | Temp: ") + temperature_Celsius+String("°C | RH: ")+air_Moisture_Percent+String("% | Air Temp: ")+air_temperature_Celsius+String("°C"));
  }
